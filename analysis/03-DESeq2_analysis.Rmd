...
```{r}
make_volcano <- function(de_res, kegg, no_labels, pCutoff, FCcutoff, prefix) {

# Get subset of metabolic genes in DESeq2 results
  goi <- de_res |> filter(padj < pCutoff , entrez_id %in% kegg) |> top_n(no_labels) |> pull(symbol)
...
lapply(seq_along(res_list), function(i){
  make_volcano(res_list[[i]], kegg = unique(unlist(kegg_gene_sets)),
               labels = 20, pCutoff = 0.05, FCcutoff = 2,
